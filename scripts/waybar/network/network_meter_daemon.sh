...
export LC_ALL=C # avoid locale decimal separator issues

# load sleep builtin: avoids fork
...
iface=""
iface_counter=0
hb_counter=2  # triggers check on first iteration
hb_time=0
...
while :; do
    ...
    if (( ++hb_counter >= 3 )); then
        ...
        if [[ -f "$HEARTBEAT_FILE" ]]; then
            ...
            hb_time=$now  # no heartbeat: assume active
        fi
    fi

    # deep sleep if waybar inactive
